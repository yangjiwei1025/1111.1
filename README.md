...
| 第5層 會議層 （Session Layer）  | 負責在數據傳輸中設定和維護電腦網路中兩台電腦之間的通訊連接。  Center-aligned |
| 第4層 傳輸層 （Transport Layer） | 把傳輸表頭（TH）加至數據以形成數據包。傳輸表頭包含了所使用的協定等傳送資訊。TCP,UDP,TLS/SSL更多 | Right-aligned | Center-aligned |
| 第3層 網路層 （Network Layer） |  決定數據的路徑選擇和轉寄，將網路表頭（NH）加至數據包，以形成封包。IP（v4·v6） ,ICMP（v6）  |          ---: | Center-aligned |
